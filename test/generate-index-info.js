...
test('nested files are kept in a hierarchy as an array', (t) => {
  t.equal(treeInfoFirstIsCurrent[2].constructor, Array, 'The nested tree is still an array')
  t.equal(treeInfoFirstIsCurrent[2][0], 'nested', 'Heading is in the first position')
  t.equal(treeInfoFirstIsCurrent[2][1].length, 2, 'Second position has array of children')
  t.end()
})
